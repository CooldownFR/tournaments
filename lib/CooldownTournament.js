                ...
                this.spreadsheet.loadInfo().then(() => {
                    ...
                    sheet.loadCells().then(() => {
                        (new Promise((resolve, reject) => {
                            if (reset) { resolve({}); }
                            else { this.getResults(sheet.getCell(this.architecture[round][group].lobby_creator[0], this.architecture[round][group].lobby_creator[1]).value).then(resolve).catch(reject); }
                        })).then((results) => {
                            for (let i = 0; i < 8; i++) {
                                sheet.getCell(
